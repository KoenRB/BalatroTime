...
SMODS.Enhancement {
    key = 'fire',
    pos = { x = 3, y = 1 }, -- TODO Need new art for this
    config = { mult = 15, extra = { last_updated = 0, time_left = 0, decay_time = 30, decay_val = 3 } },
    loc_vars = function(self, info_queue, card)
        return { vars = { card.ability.mult, card.ability.extra.last_updated } }
    end,
    update = function(self, card, dt)
        ...
        if card.ability.extra.time_left <= 0 then
            ...
            if card.ability.mult <= 0 then
                -- TODO add sound effect or visual effect for card burning out
                card:remove()
            end
        end
    end,
